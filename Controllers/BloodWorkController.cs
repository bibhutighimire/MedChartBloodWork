...
namespace MedChartBloodWork.Controllers
{
    public class BloodWorkController : Controller
    {
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            //This will filter the list of bloodwork only of the specific patient who has signed in and stores them in List and sends to vIEW PAGE
            var ToBeEditedPatient = _context.BloodWork.Where(x => x.BloodWorkID == id).FirstOrDefault();
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            //This will filter the list of bloodwork only of the specific patient who has signed in and stores them in List and sends to vIEW PAGE
            var DetailsOfAPatient = _context.BloodWork.Where(x => x.BloodWorkID == id).FirstOrDefault();
        ...
        public async Task<IActionResult> Delete(int? id)
        {
            //this will assign entire data of ASPNETUSERS Table value to user. This is how we capture ID of patient using this application if they are signed in
            var user = await _userManager.GetUserAsync(HttpContext.User);
            var BloodWorkAddPatient = _context.BloodWork.Where(x => x.BloodWorkID == id).FirstOrDefault();
            //passing the specific id in the view so that delete view can be seen by user during delete event
            return View(BloodWorkAddPatient);
        }
